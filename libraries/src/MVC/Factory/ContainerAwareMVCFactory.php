 ...
 * @since  3.10.0
 */
class ContainerAwareMVCFactory implements MVCFactoryInterface, FormFactoryAwareInterface, SiteRouterAwareInterface,
                                          UserFactoryAwareInterface, MailerFactoryAwareInterface,
                                          ContainerAwareInterface
{
    ...
    public function createController($name, $prefix, array $config, CMSApplicationInterface $app, Input $input)
    {
        ...
        $controller = $this->getContainer()->buildObject(
            $className,
            ['config' => $config, 'factory' => $this, 'app' => $app, 'input' => $input]
        );
    ...
    public function createModel($name, $prefix = '', array $config = [])
    {
        ...
        $model = $this->getContainer()->buildObject($className, ['config' => $config, 'factory' => $this]);
    ...
    public function createView($name, $prefix = '', $type = '', array $config = [])
    {
        ...
        $view = $this->getContainer()->buildObject($className, ['config' => $config]);
    ...
    public function createTable($name, $prefix = '', array $config = [])
    {
        ...
        $table = $this->getContainer()->buildObject($className, ['db' => $db]);
    ...
    private function setFormFactoryOnObject($object)
    {
        ...
        // Avoid overriding an existing form factory
        if ($object->getFormFactory() instanceof FormFactoryInterface) {
            return;
        }
    ...
    private function setDispatcherOnObject($object)
    {
        ...
        if ($object->getDispatcher() instanceof DispatcherInterface) {
            return;
        }
    ...
    private function setRouterOnObject($object): void
    {
        ...
        if ($object->getSiteRouter() instanceof SiteRouter) {
            return;
        }
    ...
    private function setCacheControllerOnObject($object): void
    {
        ...
        if ($object->getCacheControllerFactory() instanceof CacheControllerFactoryInterface) {
            return;
        }
    ...
    private function setUserFactoryOnObject($object): void
    {
        ...
        if ($object->getUserFactory() instanceof UserFactoryInterface) {
            return;
        }
    ...
    private function setMailerFactoryOnObject($object): void
    {
        ...
        if ($object->getMailerFactory() instanceof MailerFactoryInterface) {
            return;
        }
