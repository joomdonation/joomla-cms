...
class FormController extends BaseController implements FormFactoryAwareInterface
{
    ...
    public function cancel($key = null)
    {
        ...
        if ($recordId && $table->supportCheckin() && !$this->attemptCheckin($model, $recordId, $key)) {
            return false;
        }
    ...
    public function edit($key = null, $urlVar = null)
    {
        ...
        if ($table->supportCheckin() && !$this->attemptCheckout($model, $recordId, $urlVar)) {
            return false;
        }
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        if ($task === 'save2copy') {
            ...
            if ($table->supportCheckin() && !$this->attemptCheckin($model, $recordId, $urlVar)) {
                return false;
            }
        ...
        // Allow plugins to normalise/preprocess the data.
        ...
        if ($validData === false) {
            ...
            $this->handleSaveDataValidationErrors($model->getErrors());

            // Apply filter to calendar fields to avoid time shifts when displaying the form again.
        ...
        // Call the preSaveHook method to allow for the child class to acce and modify validated data before sav
     ...
     *
     * @param   FormModel   $model     The model object.
     ...
     * @since   __DEPLOY_VERSION__
     */
    protected function attemptCheckin(FormModel $model, $recordId, $urlVar): bool
    {
        if ($model->checkin($recordId) === false) {
            // Check-in failed, go back to the record and display a notice.
            $this->setMessage(Text::sprintf('JLIB_APPLICATION_ERROR_CHECKIN_FAILED', $model->getError()), 'error');

            $this->setRedirect($this->getRedirectToItemUrl($recordId, $urlVar));
     ...
     * @since   __DEPLOY_VERSION__
     */
    protected function attemptCheckout(FormModel $model, $recordId, $urlVar): bool
    {
     ...
     * @since   __DEPLOY_VERSION__
     */
    protected function handleSaveDataValidationErrors(array $errors): void
    {
        // Push up to three validation messages out to the user.
    ...
    {
        $langKey = $this->text_prefix
            . ($recordId === 0 && $this->app->isClient('site') ? '_SUBMIT' : '')
            . '_SAVE_SUCCESS';

        $prefix  = $this->app->getLanguage()->hasKey($langKey) ? $this->text_prefix : 'JLIB_APPLICATION';

        $this->setMessage(
            Text::_($prefix . ($recordId === 0 && $this->app->isClient('site') ? '_SUBMIT' : '') . '_SAVE_SUCCESS')
        );
    }
